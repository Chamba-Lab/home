...
### Site Structure
*   **Páginas**: `/` (hero + intro + CTA), `/recursos` (guías y plataformas curadas), `/eventos` (formatos de sesiones recurrentes), `/dinamicas` (hub con grilla de mini actividades comunitarias) y una subruta por actividad (ej. `/dinamicas/ruleta`).
...
*   **Dinámicas**: `/dinamicas` es un hub que lista actividades (`activities` en `src/content/dinamicas.json`) como tarjetas; cada actividad "available" enlaza a su propia subruta (`src/pages/dinamicas/<id>.astro`), manteniendo la isla React fuera del hub. Primera implementada: **La Ruleta** (`/dinamicas/ruleta`) — isla `src/components/RouletteWheel.tsx` con dos modos (tres presets precargados: Empleabilidad, Debates Tech, Qué Aprender 2026; o modo libre con opciones personalizadas, máx 12). Canvas 2D animado con easing; la rotación se persiste en un ref para no "reiniciarse" visualmente entre renders. Datos de presets editables en el JSON sin tocar código React.
